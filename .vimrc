...
"编辑一个文件时，直接用相应的缩进风格
if has("autocmd")
    "autocmd FileType go,make set sw=8 sts=8 ts=8 smarttab
    "autocmd FileType c,cpp,java,groovy,ant set sw=4 sts=4 ts=4 expandtab
    autocmd FileType c,cpp,go,make,text set sw=8 sts=8 ts=8 smarttab
    ...
    autocmd FileType lisp,scheme set sw=2 sts=2 ts=2 nocindent lisp expandtab
endif
...
func! Vimgrep()
    exec "vimgrep ".expand('<cword>')." ".split(&path, ',')[0]
    exec "normal "
    exec "copen"
endfunc
